...
const InitialLayout = () => {
  ...
  useEffect(() => {
    ...
    const isTakePhotoRoute = segments[0] === 'take-photo';

    // don't navigate until loading completes
    if (loading) return;

    console.log('InitialLayout effect', { user: !!user, segments, loading, inTabsGroup, isTakePhotoRoute });

    // Allow navigation to take-photo route without interference
    if (isTakePhotoRoute) return;
    ...
    if (user && !inTabsGroup && segments[0] === undefined) {
      ...
      router.replace('/');
    } else if (!user && !inTabsGroup && segments[0] === undefined) {
